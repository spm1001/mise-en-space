...
- **Sheets editing** (lirugi; range-aimed since mise-vadoko): `do(overwrite)` on a spreadsheet takes CSV plus optional `range=` in A1 notation — bare tab name (`"Costs"`) clears and replaces that tab, bounded range (`"Costs!F9:F15"`) writes exactly those cells, anchor (`"Costs!F9"`) spills the CSV's shape from there; USER_ENTERED semantics (formulas parse, `=HYPERLINK` renders). Un-ranged: single-tab sheets get whole-tab replace (symmetric with create); **multi-tab sheets REFUSE and teach `range=`** — a deliberate contract change from the old first-tab-plus-warning behaviour. `do(replace_text)` does literal cell find/replace across ALL tabs (matchCase, formulas excluded, `occurrences_changed` cue). `prepend`/`append` on sheets still reject, naming those two alternatives.
...
-m pytest` ends with `2205 passed, 100 deselected in 23.80s` (count as of 2026-08-08, late evening — it moved five times that day) as the last line of ~92, because
