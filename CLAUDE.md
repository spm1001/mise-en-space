...
| **3 verbs not 17 tools** | search, fetch, do | v1 had 17 tools. Claude doesn't need that many levers. Unified search + polymorphic fetch covers 95% of use cases. The 3rd verb is `do(operation=...)` — routes via operation param. Live operations (Feb 2026): `create`, `move`, `overwrite` (wholesale doc replacement with heading styles), `prepend`, `append`, `replace_text` (surgical edits). |
| **Move: single-parent enforcement** | Remove all parents, add destination | Google Drive technically supports multi-parent but it causes confusion. Move is a true move, not "add another parent". Validates destination is a folder (MIME type check) before attempting — clear `INVALID_INPUT` error if not. 2 API calls: validate destination + get name → get current parents → update. Validation call also provides folder name for cues, saving a round-trip vs the old 3-call flow. |
...
| **do() input signature: growing pains** | 11 optional params, most irrelevant per operation | `do()` routes 6 operations via a single MCP tool with `operation` param. Each operation uses a different subset of params (`content` means different things for create vs replace_text). Works today but fragile — the next operation will make it confusing. Refactor candidate: per-operation param validation at the router, or kwargs dispatch. Response shape is normalised (mise-bicesi); input shape is not yet. |
...
| **Deposit-then-publish: source param** | `do(source="mise/...")` reads from deposit | Instead of passing content inline (burns tokens), caller writes to `mise/` deposit folder, then `do(source=path)` reads content.md (doc) or content.csv (sheet) from it. Title falls back to manifest.json. After creation, manifest is enriched with `status`, `file_id`, `web_link`, `created_at` as receipt. Inline `content` param stays for backwards compatibility. Both source and content → error (ambiguous). **Contract:** `_SOURCE_FILENAME` dict in `tools/create.py` maps doc_type → expected filename. If per-tab deposit convention changes (mise-ponico), this dict must update too. |
...
| **Per-tab CSV deposits** | `content_{tab_slug}.csv` per tab, combined `content.csv` kept | A Claude missed that a single CSV contained multiple tabs separated by `=== Sheet: Name ===` headers. Per-tab files are unambiguous — each file is one tab, filename tells you which. Only written for multi-tab spreadsheets (2+ tabs); single-tab gets just `content.csv`. `extract_sheets_per_tab()` in extractors/sheets.py produces the raw per-tab data; `_write_per_tab_csvs()` in tools/fetch/drive.py writes them. Manifest gets `tabs: [{name, filename}]` array; cues get `tab_count` + `tab_names`. Same path covers both Google Sheets and XLSX (via `OfficeExtractionResult.spreadsheet_data`). `_SOURCE_FILENAME` in create.py still maps to `content.csv` (combined) — multi-tab creation reads per-tab files individually. |
| **Multi-tab sheet creation: hybrid path** | CSV upload for tab 1, Sheets API for additional tabs | `_create_multi_tab_sheet()` in tools/create.py. Tab 1 uses CSV upload (fast, 94% type detection by Drive). Tab 1 is renamed from CSV default to actual tab name via `rename_sheet()`. Additional tabs added via `add_sheet()` + `update_sheet_values()` with `USER_ENTERED` input option — this preserves formulae (cells starting with `=`) and auto-detects dates/numbers. Detection: `do_create` checks manifest for `tabs` array; if present, routes to multi-tab path. Single-tab deposits (no `tabs` in manifest) still use the fast CSV-only path. Adapter functions in adapters/sheets.py: `add_sheet()`, `update_sheet_values()`, `rename_sheet()`. |
| **XLSX: raw file deposit** | Original `.xlsx` alongside CSV | CSV is lossy (formulae, formatting, cell types gone). `fetch_and_extract_office` captures raw bytes; tool layer writes the original filename (e.g., `Budget 2026.xlsx`) to deposit — consistent with Gmail attachment naming. Manifest gets `raw_file` field. Cues surface it via file listing; `formula_count` tells callers when the raw file matters. On the large-file streaming path, raw bytes are read from temp before unlink — briefly doubles memory for files near the 50MB threshold. |
| **CSV tick-prefix: display hint not data** | Sheets API returns plain value | CSV tick-prefix (`'00412`) tells Drive to treat the cell as text, preserving leading zeros. But the tick is a display hint — `values().get()` with default `FORMATTED_VALUE` returns `"00412"`, not `"'00412"`. Confirmed Feb 2026 via integration test. Implication: round-tripping (fetch→create) loses the tick, but the value survives. |
