...
**`mcp[cli]` rides the 2.x line since 2026-08-23, and sync tool handlers are deliberately serialized.** The old `<2.0.0` ceiling was a deliberate hold while SDK 2.x was unshipped (bon `mise-veraja`, `mise-kofoba` — correct when written); adopted once 2.0.0 shipped and v1 went security-fixes-only, after the port measured at five lines with the suite green (bon `mise-vucufu`). Two consequences worth knowing. (1) **v2 runs sync `def` tool handlers on anyio worker threads** — v1 ran them inline on the event loop, one at a time — so server.py's `_serialized` decorator (a plain `threading.Lock`) keeps tool bodies one-at-a-time until the thread-safety audit deletes it deliberately (`mise-vucufu` step 2; the audit targets are deposit-folder wipe races, token-refresh stampede, calls.jsonl appends). Do NOT swap the lock for an anyio limiter cap: the stdio transport's blocked stdin read shares that pool, and capacity 1 starves tool bodies into deadlock (measured 2026-08-23). `tests/unit/test_server_concurrency.py` holds the pin test, the control that proves the instrument, and the seam assertion that the decorator kept the tool schemas. (2) **The worker thread has no running event loop, which un-broke `asyncio.run()`** in `adapters/gmail_browser.py` and `adapters/cdp.py` — dead through the live envelope on all of v1, silently, because both sit in fail-open except blocks (bon `mise-wamoco` tracks the live re-verify). The v2 SDK serves 2025-era clients (Claude Code today) from the same server with nothing to configure.
