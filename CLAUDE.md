...
| **Move: single-parent enforcement** | Remove all parents, add destination | Google Drive technically supports multi-parent but it causes confusion. Move is a true move, not "add another parent". No folder validation — trusts caller to pass a folder ID. 3 API calls: get current parents → update → resolve destination name for cues. |
