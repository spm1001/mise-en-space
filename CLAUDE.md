...
| **Search: preview in response** | Top 3 results per source inline, full set on disk | Same blind test: test Claude guessed wrong field name (`threadId` vs `thread_id`) when writing jq queries against deposited JSON. Root cause: forced to guess schema of file it hadn't read. Fix: `preview` block with top 3 Drive (name, id, mimeType) and top 3 Gmail (subject, thread_id, from). Often eliminates need to read deposited file. ~100-150 tokens. |
