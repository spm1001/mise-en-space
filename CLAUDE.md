...
- **Gmail body selection is plain-text-first with one exception** (mise-voteki): when a multipart message's HTML part holds a data grid (≥2 rows × ≥2 cells — `has_data_table` in `html_convert.py`), the body comes from converted HTML instead, because composers' auto-generated plain alternatives flatten tables to bare lines of cell text (Outlook, live case thread 19fb9faca1565748). The swap is always disclosed in `cues.warnings`; it never fires in slim builds (tag-stripping fallback would lose the table too). Policy lives in `select_body_text()` — the adapter just calls it.
