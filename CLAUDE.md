...
| `gmail_browser.py` | a-family (self-sent) URL resolution via a logged-in CDP Chrome — background tab, DOM harvest, fail-open. NB dead through the envelope on all of mcp 1.x (`asyncio.run` on the loop thread, swallowed by the fail-open) and repaired by 2.x's worker threads — first live production success 2026-08-23, three-arm verified (mise-wamoco) |
...
- **Drive search runs `corpora=allDrives` since mise-jefaki (2026-08-24).** Before that, a Shared Drive file the user never created/opened/was-directly-shared was invisible to EVERY name and fullText query while a parents-scoped listing found it — `includeItemsFromAllDrives` admits shared-drive items but never widens the searched corpus past the default `user` corpus, and parents filters widen implicitly, which is what made the zero look like term semantics ("hyphenated filenames are invisible" was this, measured false on the incident's own artefact — `docs/research/2026-08-24-jefaki-name-probe/`). The trade is Google's `incompleteSearch`, cued as `drive_incomplete` (their abandonment; distinct from `drive_truncated`, our cap). And `name contains` matches whole TOKENS (split on punctuation/space/letter–digit boundaries, AND in any order; only a literal whole-name prefix substring-matches) — a zero-hit name term carrying a separator draws the `drive_name_semantics` teaching cue instead of reading as absence.
...
- `search` drive results and every Drive deposit manifest carry `last_modified_by` (mise-tanoti): Shared Drive files have NO owners, so the last-modifier is the only honest author signal — Garni's MANIFEST 'modified <date> by <who>' line renders from mise data alone.
...
- `fetch` accepts `crops=False` — skips PDF embedded-graphic crop extraction and its content.md `exhibit:` anchors (mise-tanoti; anchors ride result.crops, so nothing dangles). Measured on the 10-PDF Garni corpus (tube, 2026-08-24): 325s → 32s (10.2×) and 122MB → 2.5MB of deposits, extracted text identical modulo anchor blocks — crop extraction was 90% of a text-only corpus walk. Default unchanged; the two levers compose.
- **PDF page-citation fidelity is measured, never inferred** (mise-wujoga): every PDF deposit carries `page_markers` (form feeds counted in content.md) and `pdf_pages` (poppler's count via `adapters/pdf_info.py` — a direct pdfinfo subprocess since 2026-08-24: it used to ride the pdf2image wrapper from the extraction EXTRA, so every core/library install got None with the binary right there on PATH (mise-tanoti); None now means poppler itself is absent), and a warning cue fires when per-page citations can't be derived (0 markers on a multi-page PDF, partial markers = misalignment, or the Drive path on a build with no page count). Marker survival is per-PDF, not per-path — markitdown kept the two-page fixture's form feed and dropped all 255 of the BBC annual report's in the same environment — so gating on `extraction_method` is wrong on real documents; that design was falsified live before shipping.
...
- **Doc tabs** (mise-wisuzu, 2026-08-24): `do(append, file_id=…, content=…, tab='Title')` places content in a NEW tab of an existing Google Doc — the non-destructive home for a parallel redraft. Two sequential batchUpdates in `tools/doc_tabs.py` (addDocumentTab → insertText by the server-minted tabId) — NEVER one batch: the single-batch shape returns 200 and silently writes the ORIGINAL tab (probed, `docs/research/2026-08-24-givige-tab-probe/`), and a caller-supplied tabId is a categorical 400. Tab content is **plain text** — Drive's markdown import cannot target a tab (aimed at a multi-tab doc it flattens the whole doc to one tab, measured live), which is also why `do(overwrite)` now REFUSES multi-tab Docs outright, teaching `append tab=` (fail-open with a warning cue when the tabs read errors — a Drive-scoped credential can write where a Docs read is refused). Cues: `tab_id`/`tab_title`/`tab_index`, an always-on plain-text note, a duplicate-title warning; `web_link` carries `?tab=<id>`. `tab=` on a spreadsheet refuses (sheet tabs are mise-dahune's surface, unbuilt).
...
**`mcp[cli]` rides the 2.x line since 2026-08-23, and tool bodies run CONCURRENTLY since 2026-08-24.** The old `<2.0.0` ceiling was a deliberate hold while SDK 2.x was unshipped (bon `mise-veraja`, `mise-kofoba` — correct when written); adopted once 2.0.0 shipped and v1 went security-fixes-only, after the port measured at five lines with the suite green (bon `mise-vucufu`). Three consequences worth knowing. (1) **v2 runs sync `def` tool handlers on anyio worker threads** — v1 ran them inline on the event loop, one at a time. The migration shipped with an interim `_serialized` lock pinning bodies one-at-a-time; the thread-safety audit (`mise-bapije`, plus a cold review that extended it) deleted the lock and put guards where the shared state lives: a per-resource deposit lock in `workspace/manager.py` (taken by the fetch dispatch, the gmail message→thread rescue, and every do() `source=` reader), O_EXCL search-deposit naming, and a single-flight refresh + locked singleton mint in `adapters/http_client.py`. Measured through the real stdio server: parallel 3-fetch 5.14s → 2.36s median. Do NOT re-serialize with an anyio limiter cap: the stdio transport's blocked stdin read shares that pool, and capacity 1 starves tool bodies into deadlock (measured 2026-08-23). `tests/unit/test_server_concurrency.py` holds the interleave control and the no-wrapper seam; `tests/unit/test_concurrency_guards.py` hammers the guards, and each was seen to FAIL with its guard removed before its green was trusted. (2) **The worker thread has no running event loop, which un-broke `asyncio.run()`** in `adapters/gmail_browser.py` and `adapters/cdp.py` — dead through the live envelope on all of v1, silently, because both sit in fail-open except blocks (bon `mise-wamoco` tracks the live re-verify). (3) **The residual concurrency scope is honest, not total:** cross-process races (two servers, one cwd) were never covered by the old lock either, and the deposit locks are in-process only. The v2 SDK serves 2025-era clients (Claude Code today) from the same server with nothing to configure.

**Two v2 capabilities are deliberately NOT adopted (mise-huwubi verdicts, probes 2026-08-24 in `docs/research/2026-08-24-huwubi-probes/`).** (1) **Teaching errors stay in-band** — v2 lets tools raise MCPError with structured data to the protocol layer, but mise's error dicts are FOR the calling model (they teach the next move), and the funnel contract above already guarantees nothing reaches a response raw. Deliberate retention, not oversight. (2) **Typed output schemas are parked** — measured: CC injects only the JSON text block into the model's context (structuredContent never reaches it), so a typed cues contract would buy nothing model-visible today while growing `tools/list` on exactly the axis where mise's Tier-0 margin lives (outputSchema is 73–76% of Google's fleet cost). Related measured fact: since the 2.x migration, v2's return-annotation auto-detect mints a permissive ~75-byte outputSchema on each of mise's three tools (`dict[str, Any]` → open object) — harmless, but any Tier-0 re-harvest will see it; suppressing via `structured_output=False` buys ~226 bytes and was judged not worth the ceremony. Revisit the park if CC starts exposing structuredContent to the model as separate structured content — "consuming" is the wrong trigger, since the cold review's stream-json check showed CC re-serializes to compact JSON (it may already consume the field internally); what the model *receives* is the quantity that matters, and mechanically it receives one text string.
...
| **Google Docs eats `\f` and `\x00` on every write path — and `\f` is mise's own page marker** | Measured live 2026-08-24 on five scratch docs (`docs/research/2026-08-24-melaso-control-chars/`, read back through both `documents.get` and the `text/plain` export). The **markdown import** (`create`, `overwrite`) deletes form feeds and **TRUNCATES the document at the first NUL** — `BEFORE\x00AFTER` imported as a doc ending at `BEFORE`, everything after it gone under an HTTP 200 (the control with a plain `X` kept its tail). **insertText/replaceAllText** (`prepend`, `append`, `append tab=`, `replace_text`) delete both characters but keep the rest. `\r\n` normalises, `\t` and `\v` survive — and `\v` must, because `convert_fenced_blocks` relies on it. The sharp edge is that `pdftotext` separates pages with `\f`, mise counts those into `cues.page_markers`, so pasting a PDF deposit into a Doc silently lost every page boundary while the fetch cue still promised page citations. `tools/doc_control_chars.py` now converts each form feed to a `---` line before the write — probed to import as a real `horizontalRule`, so it survives *visibly* — strips NULs before they can truncate, and cues `page_breaks_marked` / `nuls_removed` plus a warning every time. Both transforms sit BELOW the plain-file routing: a byte upload preserves both characters (measured), and that is the lossless route. Sheets go up as `text/csv` through a different engine and are **unmeasured** (mise-melaso). |
...
| **`comments.md` locates comments — all three surfaces, from two different planes** | **Docs** correlate on *content*: `_enrich_with_comments` passes the fetched markdown to the comments extractor, which matches each comment's anchor text against the document tree (nearest heading › sub-group). **Slides and Sheets** correlate on *ids*: a second GET with `commentsViewMode=COMMENTS_VIEW_MODE_INCLUDED` (`adapters/comment_anchors.py`) returns anchorId → slide objectId / GridRange, resolved to `slide N (title)` and `Tab!B12` by `extractors/comment_anchors.py`. Either way comments render in **document order** with a `↳` locator, and container-scoped or orphaned anchors are flagged `⚠`. Three things not to get wrong. (1) **The join is outer and one-way** — the Drive comments plane decides which comments exist; the anchor map can only *add* a locator, never drop a thread. Do not "optimise" it into an inner join: losing a comment to the void is the failure this was built against. (2) **Slides/Sheets locators ride a Developer Preview**, so every failure degrades to today's flat render plus a `cues.warnings` line naming the reason — that degradation is a DPP term-(iv) compliance requirement (pre-GA features must not ship to customers) and mise is publicly distributed, so don't make any default render *depend* on the preview. (3) **The second read is paid only when the file has open comments** (checkbox-oracle pattern), and only for Slides/Sheets — the Docs path makes no extra call. Anchor text is HTML-unescaped; multi-line span anchors quote every line; on Slides/Sheets the preview's `plainTextQuote` supplies anchor text where the Drive plane has none. Probe evidence: `docs/research/2026-08-31-anchored-comments-probe/` (mise-picihi → mise-dukacu). |
| **The comments read is capped at 100 threads, and the cap used to eat the open ones** | `_enrich_with_comments` asks for 100. Until 2026-09-01 the `include_resolved=False` filter ran *after* that cap, so a file whose first 100 threads in API order were resolved returned **zero** open comments — no `comments.md`, `open_comment_count` 0, no warning, and nothing to distinguish it from a file nobody has commented on. The filter now runs inside the pagination loop so the cap counts what is kept, and a `nextPageToken` surviving at the cap sets `FileCommentsData.truncated`, which turns the header into `(100+ — capped, this file has more)` plus a warning — the same evidence contract as `SearchResults.truncated` (mise-werevi), because `len(comments) == max_results` can never tell an exactly-full read from a capped one. Both facts are observable only inside that loop, which is why `adapters/drive.py` took an argued baseline raise rather than a sibling extraction. |
