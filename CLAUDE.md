...
| **XLSX: raw file deposit** | `source.xlsx` alongside CSV | CSV is lossy (formulae, formatting, cell types gone). `fetch_and_extract_office` captures raw bytes; tool layer writes `source.xlsx` to deposit. Fixed name (not original filename) for predictability — like `content.csv`. Manifest gets `raw_file` field. Cues surface it via file listing; `formula_count` tells callers when the raw file matters. On the large-file streaming path, raw bytes are read from temp before unlink — briefly doubles memory for files near the 50MB threshold. |
