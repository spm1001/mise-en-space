...
| `replace_text` | Find and replace text — applies across ALL tabs, Docs and Sheets alike (Sheets: formulas untouched) | `file_id`, `find`, `content` |
...
| `doc_type` | str | 'doc' | create ('doc', 'sheet', 'file', 'folder', 'form'). 'file' uploads as-is — MIME inferred from title extension. 'folder' creates an empty folder (no content needed). 'form' creates a Google Form from a YAML/JSON spec (`content` or `file_path`; `folder_id` places it). |
...
| `destination_folder_id` | str | None | move (deprecated alias for `folder_id`; both naming different folders refuses) |
...
| `cc` | str | None | draft, reply_draft (CC addresses, comma-separated; with reply_all they are added to the inferred Cc) |
| `include` | list[str] | None | draft, reply_draft (Drive file IDs — resolved to formatted links in body) |
| `reply_all` | bool | False | reply_draft (if True, Cc all original recipients, plus any `cc`) |
...
| `meet` | bool | None | create_event, update_event (True mints a fresh Meet link; on update_event False removes it) |
