...
**Create_event** books an event on the user's primary calendar. `time_min`/`time_max` are the start and end — ISO datetimes (naive ones mean wall-clock time in the user's own timezone, resolved from their diary) or two bare dates for an all-day event (end date exclusive, Google's rule). With `attendees`, the call is a two-step confirm gate: the first call returns a preview with a clash check against the user's own diary — show it, then call again with `confirm=True` to book and send invites in one go (invite-first: `send_updates` defaults to `all`; the invite IS the proposal). Without attendees it books directly. Extras: `content` = description, `location`, `meet=True` mints a fresh Meet link, `recurrence='RRULE:FREQ=WEEKLY;INTERVAL=2;BYDAY=TU'` makes a series (a weekly BYDAY that skips the start's own weekday draws a warning — Google keeps the start as a stray extra instance), `include=[Drive file ids]` attaches files to the event, `properties={'mise:programme': '1to1-2026'}` adds queryable key-values (extendedProperties.private — UI-invisible; every mise-minted event also carries mise:minted_by/minted_at stamps automatically, so a reconciler can find programme events by key regardless of title edits). There is no delete op: a mis-booked event is removed in the Calendar UI, which is why the gate previews first.

**Update_event** edits an event — `file_id` is the Calendar event id or the invite's Gmail thread id (resolved like `respond`). Gate grain is blast radius: **structural** changes (both `time_min`+`time_max` to move it, `recurrence` — including converting a single event into a series, `attendees` which are always ADDED never removed, `meet=True`) preview first and email attendees on confirm (`send_updates` default `all`); **cosmetic** changes (`content` = replace description, `title`, `location`, `include` = add attachments, `properties` = add/overwrite queryable key-values — existing keys merge, never wiped, so it backfills stamps on pre-existing events) execute directly and quietly (`send_updates` default `none`). `cues.previous` carries the old values — events have no version history, so that cue is the undo reference. Guests can only add attendees (when the organiser allows it); everything else needs the organiser.
