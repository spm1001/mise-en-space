...
| `overwrite` | Replace full file content (Google Doc or plain file; Sheets: CSV content, `range=` aims a tab or cells; Forms: YAML/JSON spec replaces all questions) | `file_id`, `content` OR `source` OR `file_path` (bytes as-is for plain files: PDF, PNG…, id kept) |
...
**Pasting a PDF deposit into a Doc keeps its page boundaries** (2026-08-24, mise-melaso). Google Docs deletes form feeds on *every* write path — measured, both the markdown import and the `insertText` one — and a form feed is exactly how `content.md` marks PDF page breaks, so this used to lose every boundary in silence. Mise now converts each one to a `---` line before writing (a real horizontal rule after `create`/`overwrite`, a literal `---` on `prepend`/`append`/`tab=`, which are plain text) and cues `page_breaks_marked`. NUL bytes get the sharper treatment: they are stripped, because the markdown import **truncates the document at the first one** and discards everything after it under an HTTP 200 — on an `overwrite` that means the old content is already gone and only the head of the new content arrives. `cues.nuls_removed` says how many, and a NUL in text content is usually a sign the source was binary or mis-decoded. Plain files (`doc_type='file'`, and `overwrite` on a `.md`) keep both characters untouched — that is the lossless route when the exact bytes matter.
