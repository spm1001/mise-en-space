...
**Markdown footnotes become real Docs footnotes** (since suite 1.75, 2026-08-24, mise-rubucu): `[^N]` anchors with matching `[^N]: definition` lines in `create`/`overwrite` doc content land as native footnotes — superscript reference, text in the footnote pane — and fetch renders them back as `[^N]` + definitions, so the round-trip holds. `cues.footnotes_inserted` confirms; orphan anchors, orphan definitions, duplicate anchors and pass failures stay as literal text, named in `cues.footnote_errors` (2026-08-24: content is never silently lost — every failure path restores the stripped definitions).
