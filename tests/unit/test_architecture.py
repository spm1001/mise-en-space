...
_LEGACY_SIZE_BASELINE = {
    ...
    "adapters/http_client.py": 722,  # +11 (2026-08-12): ambient-mode dispatch — a 4-line branch in _load_and_diagnose_credentials plus a 5-line refresh guard in EACH near-duplicate client (mise-wasagu). +9 (2026-08-12 evening, mise-dareti): constructor-injected credentials pay the SAME three seams — a 3-line return in the loader, a 3-line refusal in each refresh path; the registry and teaching text live in token_store. These are the only seams identity selection can intercept. +19 (2026-08-24, mise-bapije): single-flight refresh lock in the SYNC client — tool bodies run concurrently since the serializer lift, and the dead-grant reload path swaps self._credentials; most of the 19 is the comment saying why the async client deliberately gets none. Halves when MiseSyncClient dies in Phase 2.
