...
_LEGACY_SIZE_BASELINE = {
    "adapters/drive.py": 1156,  # +5 (2026-08-24, mise-tanoti): lastModifyingUser in BOTH fields masks + the search-parse extraction — mask lines and their parser can only live in this file (no sibling owns a fields mask), and Shared Drive files have no owners, so this is the manifest's only author signal
