...
_LEGACY_SIZE_BASELINE = {
    "adapters/drive.py": 1189,  # +15 (2026-09-01, mise-dukacu): comments.list honesty — the resolved filter moves INSIDE the pagination loop (it ran after the cap, so 100 resolved threads could exhaust max_results and return zero open ones: a file with live comments deposited NO comments.md, indistinguishable from a file with none — measured by the essayeur) and a surviving nextPageToken at the cap now sets FileCommentsData.truncated with a warning. Both facts exist ONLY in this loop: no sibling can see the token or the per-page resolved flags, which is the same argument as the mise-jefaki incompleteSearch entry below. Most of the 15 are the comment saying why, plus one indent level on the CommentData block. +5 (2026-08-24, mise-tanoti): lastModifyingUser in BOTH fields masks + the search-parse extraction — mask lines and their parser can only live in this file (no sibling owns a fields mask), and Shared Drive files have no owners, so this is the manifest's only author signal; +18 (2026-08-24, mise-jefaki): corpora=allDrives at the params-build site + incompleteSearch in the mask, captured per page, returned on the model — params dict and fields mask can only live here; the cue text and the measured rationale live in tools/search_drive_cues.py and docs/research/2026-08-24-jefaki-name-probe/
    "adapters/gmail.py": 1064,  # tightened 2026-09-23 (mise-sajeso): the attachment list-comps folded into _as_attachments while hidden_attachments was added; +2 earlier the same day (mise-mudupa) for the with_attachments call site — MIME wrapping lives in adapters/gmail_draft_attachments.py. Earlier: tightened 2026-08-07: id resolvers split to gmail_ids.py
    "tools/create.py": 910,  # tightened 2026-09-23 (-42, mise-suwopu): multi-tab sheet finishing moved to tools/sheet_create.py, where the rename-by-real-sheetId and create-all-tabs-first fix landed. Earlier: +3 (2026-09-22, mise-tijeko): call-site wiring only — the folder and form early-returns now hand their doc-shaped params on (a warning for folders, file_path/folder_id consumed for forms) instead of dropping them in silence; the logic lives in tools/folder_cues.py and tools/form_create.py. The early-returns can only live here, where the params arrive. Earlier: tightened 2026-08-09 thrice: find_placeholder_indices moved to doc_chips.py, de-aliased imports (mise-rafote), csv_text_to_values moved to extractors/sheets.py (mise-kacani). +11 (2026-08-24, mise-bapije cold review): source reads and enrich_manifest under the per-resource deposit lock — unlocked they raced a concurrent fetch's wipe-and-rewrite (torn read / stale-manifest clobber); +8 (2026-08-24): footnote wiring — logic lives in tools/doc_footnotes.py (mise-rubucu); +8 (2026-08-24, mise-melaso): control-character wiring — one import, one gated call, one cue fold; all 150 lines of logic live in tools/doc_control_chars.py. The call cannot move to a sibling: it must run BEFORE the image/chip/footnote passes (footnote text goes up by its own insertText and is eaten the same way), and the cue must land on the DoResult this function builds. Same shape and same size as the rubucu entry above it
    "extractors/docs.py": 895,  # +3 (2026-08-24): footnote-separator accretion fix comment (mise-rubucu)
    "tools/fetch/drive.py": 809,  # tightened 2026-08-08: _write_per_tab_csvs moved to common.py (mise-dogape); +1 (2026-08-13): pdf_page_fidelity spread in fetch_pdf's manifest extras — logic lives in common.py (mise-wujoga); +2 (2026-08-18): deposit_pdf_crops call + spread — logic lives in common.py (mise-jopohi); +1 (2026-08-24): write_figures call — logic lives in workspace/manager.py (mise-gerefe)
    "resources/docs.py": 904,  # +30 for the 'people' search source (mise-mahiho); the last 6 are the multi-word query trap, added after a live probe showed `orgTitle:Head of Strategy` returns zero SILENTLY — a caller who doesn't know that reads the zero as "nobody has that job" — this module's mass IS resource text, so its ceiling tracks CAPABILITY additions; splitting a resource string across siblings would be worse. Raise only with a new op or search source, and only by what the new capability's grammar actually needs. The people entry earns its lines on query grammar (Admin SDK syntax, not Drive's) plus two honesty notes the caller cannot infer. +3 (2026-08-14): the fetch param table backfilled recursive/raw and gained thumbnails — closing a self-disclosure gap found at /close (the table had silently lagged the signature). +18 (2026-08-16): the calendar window (mise-riduka) — two param rows plus a section whose lines are the capability's grammar: whole-day widening of bare dates, overlap semantics, and the two overflow modes a caller cannot infer from the truncation cue alone. +23 (2026-08-19): three calendar-write ops (mise-rijeco) — table rows, a params table, and three prose blocks whose lines are the ops' contract: gate grain (structural vs cosmetic), the not_visible/location honesty cues, and the no-delete boundary. +1 (2026-08-19, gujiro+kawegu): two params (properties, color) cost ONE net line — prose extended in place, two table rows; the line carries the probed label ceiling a caller cannot infer (unknown eventLabelId accepted-and-enriched, palette scope-gated). +2 (2026-08-20, writable-fields sweep): visibility + transparency rows — the transparency line carries the busy/free semantics a caller cannot infer (an opaque hold eats colleagues' slot-mining; the dry run's fiction-slots fault in reverse).; +2 (2026-08-24): footnotes paragraph in docs_do (mise-rubucu); +3 (2026-08-24, mise-wisuzu): append tab= — one param row plus a Doc-tabs paragraph whose lines are the contract a caller cannot infer: plain-text-only (the import engine flattens multi-tab docs — measured), the overwrite multi-tab refusal, duplicate-title warn-not-refuse; +1 (2026-08-24, mise-tanoti): the crops= fetch param row — carries the measured 10× and the text-identical guarantee a caller cannot infer
    "tools/fetch/gmail.py": 721, # +7 (2026-09-23, mise-sajeso + its essayeur): two import names, one cues |= hidden_attachment_cues(...) line, one assign_attachment_refs(...) call in each of the two thread-fetch paths, and a two-line duplicate skip in the eager loop — the logic lives in gmail_attachments.py; the lookup now scans hidden attachments in the same line count.  # +1 (2026-08-07): the gmail_ids split turned one import into two; +6 (2026-08-09): web_link emission, logic lives in gmail_ids.thread_web_link_or_warn (mise-hetaba); +2 (2026-08-13): thumbnails opt-out — one signature line, one if-guard at the render site (mise-giwawa); +2 (2026-08-13): pdf_page_fidelity call + spread on the attachment path — logic lives in common.py (mise-wujoga); +8 (2026-08-24, mise-bapije cold review): the deposit phase runs under a second, inner take of the deposit lock keyed on the RESOLVED thread id — a bare MESSAGE id spelling otherwise holds a different lock over the same folder and the wipe race returns; +1 (2026-08-24): write_figures call — logic lives in workspace/manager.py (mise-gerefe)
    "adapters/http_client.py": 737,  # +11 (2026-08-12): ambient-mode dispatch — a 4-line branch in _load_and_diagnose_credentials plus a 5-line refresh guard in EACH near-duplicate client (mise-wasagu). +9 (2026-08-12 evening, mise-dareti): constructor-injected credentials pay the SAME three seams — a 3-line return in the loader, a 3-line refusal in each refresh path; the registry and teaching text live in token_store. These are the only seams identity selection can intercept. +34 (2026-08-24, mise-bapije + its cold review): single-flight refresh lock in the SYNC client (the dead-grant reload path swaps self._credentials), identity-cache clears reordered BEFORE the swap so a sibling's window reads as identity-unknown rather than the old account, and the singleton mint double-checked under a lock (unlocked, a first-burst pair minted two httpx pools). Most of the lines are the comments saying why; the async client deliberately gets none of it (one loop). Halves when MiseSyncClient dies in Phase 2.
    "extractors/slides.py": 607,  # +7 (2026-08-18): the exhibit anchor on thumbnailed slides — an eye-level line in the per-slide render, which only this module can emit (mise-jopohi)
    "extractors/talon_signature.py": 522,  # +4 (2026-09-23, mise-kubolo): call-site wiring only — one import and three guard calls (the third names discarded links, essayeur follow-up) in _strip_trailing_contact_block (table furniture is not a name block; an anchored call-to-action link vetoes the strip). The guards and their rationale live in extractors/signature_guards.py; the calls can only sit inside the one loop that finds the cut.
}
