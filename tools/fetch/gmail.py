...
def fetch_gmail(thread_id: str, base_path: Path | None = None) -> FetchResult:
    ...
    # Second, inner take on the DEPOSIT lock, keyed on the RESOLVED thread id.
    # The router locked on what the caller passed; when that was a bare
    # MESSAGE id, the rescue above re-pointed thread_id at the real thread,
    # and a sibling fetching the THREAD spelling holds a different lock over
    # the same folder (cold review, 2026-08-24). RLock makes the
    # same-spelling case free; the divergent-spelling case serializes here.
    with deposit_lock(thread_id):
